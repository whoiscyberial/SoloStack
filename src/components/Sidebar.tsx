  ...
  return (
    <>
      <div
        ...
        } sticky right-0 top-0 w-full transition-all md:hidden`}
      >
        <Button
      ...
      <motion.div
        ...
        } fixed left-0 top-0 z-10 h-full min-h-full w-[256px] overflow-y-auto border-r  border-neutral-800 bg-neutral-850 px-8 py-14 transition-all md:block`}
      >
        <div className="grid grid-flow-row gap-y-14">
          {categories.map((category) => {
            return (
              <div className="grid grid-flow-row gap-y-[4px]" key={category.id}>
          ...
          <div className="grid grid-flow-row gap-y-4">
            ...
            <ToolFormButton className="text-left">Add your tool</ToolFormButton>
      ...
      {showSidebar === true && (
        <motion.div
          animate={showSidebar ? { opacity: 1 } : { opacity: 0 }}
          className="absolute right-0 top-0 z-10 h-screen w-[calc(100vw-256px)] bg-neutral-950/50 md:hidden"
